...
| | 一般 LLM App | AI Agent（Hermes） |
|---|---|---|
| 回答問題 | ✅ | ✅ |
| 寫程式碼 | ✅ 顯示在畫面上 | ✅ **直接在電腦上執行** |
| 操作檔案 | ❌ | ✅ 建立、讀取、修改檔案 |
| 定時自動執行任務 | ❌ | ✅ 排程功能 |
| 記住你的偏好 | 有限 | ✅ 跨對話記憶 |
| 從手機操控電腦 | ❌ | ✅ 透過 Telegram |

舉個例子：你跟一般 LLM 說「幫我用 Python 計算 1 到 100 的總和」，它會把程式碼**顯示**給你看。但你跟 Hermes Agent 說同樣的話，它會**直接執行程式碼**，然後把結果 `5050` 傳回給你。
...
- **開源免費**：MIT 授權，完全免費使用
- **不綁定模型**：支援 200+ 種 AI 模型，可以用免費的 Gemini API，也能跑本地模型
- **多平台串接**：一個 Agent 可以同時透過 Telegram、Discord、Slack、WhatsApp 等平台操作
- **自我學習**：會記住你的偏好，越用越懂你
- **技能系統**：遇到重複的任務會自動建立可重用的流程
- **安全隔離**：支援 Docker 沙盒，程式碼執行不會影響你的系統

### 今天的目標

完成這份教學後，你會有一個在 Telegram 上運作的 AI Agent，它能：

1. 跟你正常對話
2. 執行 Python 程式碼並回傳結果
3. 列出和操作你電腦上的檔案
4. 記住你的對話紀錄
...
- **手機**：到 App Store 或 Google Play 搜尋「Telegram」下載
- **電腦**：到 [telegram.org](https://telegram.org/) 下載桌面版（選用）

註冊帳號後，記住你的 Telegram 使用者名稱。
...
貼上你在 Part 2 取得的 Bot Token，按 Enter。
...
```
用 Python 計算 1 到 100 的總和，然後執行它
```

Bot 應該會回覆 **5050**——它不只是告訴你程式碼，而是**真的在你的電腦上執行了**。
...
**解決方式**：等 1-2 分鐘再試。每次提問之間不要太快連續傳訊息。
...
在 WSL2 裡，你的 Windows 檔案在 `/mnt/c/` 底下。例如：

- Windows 桌面：`/mnt/c/Users/你的使用者名稱/Desktop`
- Windows 文件：`/mnt/c/Users/你的使用者名稱/Documents`
...
WSL2 和 Windows 共用網路，所以 Telegram 連線不會有問題。Gemini API 也正常運作。
